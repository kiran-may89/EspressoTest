...
class GalleryIntentTest {
    ...
    @Test
    fun test_validIntent(){
        val expectedIntent = allOf(
            hasAction(Intent.ACTION_PICK),
            hasData(MediaStore.Images.Media.EXTERNAL_CONTENT_URI)
        )
        val activityResult  = getActivityResultSub();
        intending(  hasData(MediaStore.Images.Media.EXTERNAL_CONTENT_URI)).respondWith(activityResult)
    ...
    fun getActivityResultSub(): Instrumentation.ActivityResult {
        ...
        return Instrumentation.ActivityResult(RESULT_CANCELED, resultIntent)
    }
}